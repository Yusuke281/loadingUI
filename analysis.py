...
def analyze_data(filepath):
    ...
    # --- 2. 基本統計量の表示 ---
    print("--- 全体の基本統計量 ---")
    print(df[['taskDuration', 'mouseDistance', 'rageClicks']].describe())
    print("\n" + "="*50 + "\n")

    # --- 3. ローダーの種類ごとの集計 ---
    print("--- ローダーの種類ごとの平均値 ---")
    grouped_by_loader = df.groupby('loaderType')[['taskDuration', 'mouseDistance', 'rageClicks']].mean()
    ...
    if 'simulatedLoadingTime' in df.columns:
        grouped_by_loader_time = df.groupby(['loaderType', 'simulatedLoadingTime'])[['taskDuration', 'mouseDistance', 'rageClicks']].mean()
